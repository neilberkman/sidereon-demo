...
const CAP_CARDS: [string, string][] = [
  ...
  ["GNSS/INS FUSION", "Strapdown mechanization with an error-state extended Kalman filter: loose and tight coupling with per-satellite pseudorange and range-rate rows, valid from a single satellite; a full-geometry update reproduces the single-point solver, and weak geometry is bounded by the propagated motion-model prior as a tested property."],
